...
void
gn_stop_conn_mgmt_thrds (gn_wrkr_conf_s * const wrkr_conf)
{
  gn_conn_mgmt_thrd_conf_s * conf = wrkr_conf->conn_mgmt_thrd_conf_list.head;

  while (wrkr_conf->conn_mgmt_thrd_conf_list.len > 0) {
    switch (conf->state) {
      ...
      case CONN_MGMT_THRD_STOPPED: {
        ...
        gn_conn_mgmt_thrd_conf_list_remove (&wrkr_conf->conn_mgmt_thrd_conf_list, conf);
        const gn_conn_mgmt_thrd_conf_s * next_conf = conf->next;
        if (conf == conf->next) next_conf = NULL;

        free (conf);
        conf = (gn_conn_mgmt_thrd_conf_s *)next_conf;
        break;
      }
      ...
      case CONN_MGMT_THRD_RUNNING: {
        if (conf->stop) {
          conf = conf->next;
          break; // The stop signal was already sent, don't send it again.
        }
